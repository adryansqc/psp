    ...
    <section
        ...
        <template x-for="(slide, index) in slides" :key="'slide-' + index">
            <div class="absolute inset-0 bg-forest"
                ...
                <div class="hidden sm:block absolute inset-0 bg-linear-to-r from-forest/90 via-forest/40 to-transparent">
                </div>
            </div>
        </template>

        <div class="hidden sm:block relative max-w-7xl px-2 lg:px-10 text-stone z-10" data-aos="fade-up"
            x-data="{ lang: localStorage.getItem('preferred_lang') === 'en' ? 'EN' : 'ID' }">
...
@else
    <section
        ...
        <div class="hidden sm:block absolute inset-0 bg-linear-to-r from-forest/90 via-forest/40 to-transparent"></div>
